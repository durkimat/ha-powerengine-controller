...
**Version this guide matches:** 0.4.5 (beta, Passive-only)
...
```
PowerEngine 0.4.5 starting (Passive-only build: nothing is controlled)
...
fills the last 14 days from HA history within a few minutes, then counts live), and **Config**, which opens the
...
5. **Inputs:** work down each section. For every input:
   ...
   - **Battery power must have a sign.** Watch it while the battery charges: if it
     ...
     which despite their names are power in W); PowerEngine then uses out − in.
